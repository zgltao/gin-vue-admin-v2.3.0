...
func GetTables(dbName string) (err error, TableNames []request.TableReq) {
	...
	case "postgres":
		err = global.GVA_DB.Raw("SELECT tablename \"table_name\" FROM pg_tables WHERE tablename NOT LIKE 'pg%' AND tablename NOT LIKE 'sql_%' ORDER BY tablename;").Scan(&TableNames).Error
	}
...
func GetDB() (err error, DBNames []request.DBReq) {
	...
	switch global.GVA_CONFIG.System.DbType {
	case "mysql":
		strsql = "SELECT SCHEMA_NAME AS `database` FROM INFORMATION_SCHEMA.SCHEMATA;"
	case "postgres":
		strsql = "SELECT datname \"database\" FROM pg_database;"
	}
	err = global.GVA_DB.Raw(strsql).Scan(&DBNames).Error
	return err, DBNames
}
...
func GetColume(tableName string, dbName string) (err error, Columes []request.ColumeReq) {
	...
	case "postgres":
		err = global.GVA_DB.Raw("SELECT a.attname AS colume_name, t.typname AS data_type ,a.attcollation AS data_type_long , b.description AS colume_comment FROM pg_class c, pg_attribute a  LEFT JOIN pg_description b  ON a.attrelid = b.objoid  AND a.attnum = b.objsubid, pg_type t WHERE a.attnum > 0  AND a.attrelid = c.oid  AND a.atttypid = t.oid AND c.relname = ? ORDER BY a.attnum;", tableName).Scan(&Columes).Error
	}
	return err, Columes
}
